...
>   ```
> - This method works for all command-line invocations, including `mineru`, `mineru-openai-server`, `mineru-api`, and `mineru-router`, and it applies to both the `pipeline` and `vlm` backends.
